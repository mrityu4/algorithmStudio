...
for (let i = 0; i < table.length; ++i) g[i] = table[i].slice();
...
function whileloop() {
  for (let i = 0; i < fringe.length; ++i) {
    if (g[fringe[min][0]][fringe[min][1]] > g[fringe[i]][0][fringe[i][1]])
      min = i;
  }
  table[cr][cc] = true;
  document.getElementById(cr + " " + cc).classList.add("visited");

  if (!(fringe.length > 0 && !(fringe[min][0] == dr && fringe[min][1] == dc))) {
