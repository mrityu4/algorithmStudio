...
let currow;
let startset = false;
let startAddress = null, destAddress = null;
let q = [];
let queued = [];
let visited = [];
let parent = [];
let qtraversedindex = 0;
let tr, tc;
let r, c, dr, dc, clcksearch, pathtrace, currentpathnode, pathqlength, currentpathq = 0;
let pathq = [];
function clearListeners() {
  ...
  document.getElementById('play').addEventListener("click", dfs);
}
...
function addsourcedest(e) {
  ...
  if (startset) {
    ...
    document.getElementById(destAddress).innerHTML = '<i class="material-icons small left">room</i>';
    clearListeners();
  }
  else {
    ...
    document.getElementById(startAddress).innerHTML = '<i class="material-icons small left">brightness_high</i>';
...
for (let i = 0; i < table.length; ++i)
  queued[i] = table[i].slice();
for (let i = 0; i < table.length; ++i)
  visited[i] = table[i].slice();
// console.log(table);
...
async function dfs() {
  ...
  clcksearch = setInterval(whileloop, 100);
...
function whileloop() {
  if (!(q.length > 0 && !(q[qtraversedindex][0] == dr && q[qtraversedindex][1] == dc))) {
    currentpathnode = [dr, dc];
    pathquecreator();
    pathtrace = setInterval(path, 100);
    clearInterval(clcksearch);
  }
  let currentCell = q[qtraversedindex];
  let cr = currentCell[0];
  let cc = currentCell[1];
  document.getElementById(cr + ' ' + cc).classList.add("visited");

  if ((cr + 1) >= 0 && (cr + 1) < row && cc >= 0 && cc < col && table[cr + 1][cc] == -1 && visited[cr + 1][cc] == -1) {
    if (queued[cr + 1][(cc)] == -1) {
      table[cr + 1][cc] = true;
      q.push([cr + 1, (cc)]);
      visited[cr + 1][cc] = true;
      qtraversedindex = q.length - 1;
      queued[cr + 1][cc] = [cr, cc];
    }
  }

  else if ((cr >= 0 && cr < row && (cc + 1) >= 0 && (cc + 1) < col) && table[cr][cc + 1] == -1 && visited[cr][cc + 1] == -1) {
    if (queued[cr][(cc + 1)] == -1) {
      table[cr][cc + 1] = true;
      // document.getElementById(cr + ' ' + (cc + 1)).classList.add("visited");
      q.push([cr, (cc + 1)]);
  ...
  else if (((cr - 1) >= 0 && (cr - 1) < row && cc >= 0 && cc < col) && table[cr - 1][cc] == -1 && visited[cr - 1][cc] == -1) {
    if (queued[cr - 1][(cc)] == -1) {
      table[cr - 1][cc] = true;
      // document.getElementById((cr - 1) + ' ' + cc).classList.add("visited");      
      q.push([cr - 1, (cc)]);
      visited[cr - 1][cc] = true;
      qtraversedindex = q.length - 1;
      queued[cr - 1][cc] = [cr, cc];
    ...
    tr = q[qtraversedindex][0];
    tc = q[qtraversedindex][1];
    // console.log((  (tr + 1) >= 0 && (tr + 1) < row && tc >= 0 && tc < col  ),visited[tr + 1][tc] == -1);
    // console.log((  tr >= 0 && tr < row && (tc + 1) >= 0 && (tc + 1) < col  ),visited[tr][tc + 1] == -1);
    // console.log((  (tr - 1) >= 0 && (tr - 1) < row && tc >= 0 && tc < col  ));
    // console.log((  tr >= 0 && tr < row && (tc - 1) >= 0 && (tc - 1) < col  ),visited[tr][tc - 1] == -1);

    while (!(
      ((  (tr + 1) >= 0 && (tr + 1) < row && tc >= 0 && tc < col  ) && table[tr + 1][tc] == -1 && visited[tr + 1][tc] == -1) ||
      ((  tr >= 0 && tr < row && (tc + 1) >= 0 && (tc + 1) < col  ) && table[tr][tc + 1] == -1 && visited[tr][tc + 1] == -1) ||
      ((  (tr - 1) >= 0 && (tr - 1) < row && tc >= 0 && tc < col  )&& table[tr - 1][tc] == -1 && visited[tr - 1][tc] == -1) ||
      ((  tr >= 0 && tr < row && (tc - 1) >= 0 && (tc - 1) < col  ) && table[tr][tc - 1] == -1 && visited[tr][tc - 1] == -1)
    )) {
      --qtraversedindex;
...
function path() {
  ...
  if (currentpathq <= (pathqlength - 1)) {
    document.getElementById(pathq[currentpathq][0] + ' ' + pathq[currentpathq][1]).classList.add('path');
    currentpathq++;
  }
...
function pathquecreator() {
  ...
  while (!(currentpathnode[0] == r && currentpathnode[1] == c)) {
    let cpn0 = currentpathnode[0], cpn1 = currentpathnode[1];
